;(function (global) {
          ...
          if ((this.readyState == 3 || this.readyState == 4) && this.status == 200) {
            ...
            for (; i < parts.length; i++) {
              ...
              if (line.indexOf('data') == 0) {
                ...
                if (data.length) {
                  var event = new MessageEvent(data.join('\n'), eventName, eventsource.url, lastEventId);
                  console.log(eventName, data.join("\n"));
                  eventsource.dispatchEvent(eventName, event);
                  data = [];
                  eventName = '';
                }
              }
  ...
  var MessageEvent = function (data, event, origin, lastEventId) {
    this.data = data;
